...
node_out=$(node tests/runner-vault-classifier-youtube-shorts.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'vault-classifier-youtube-shorts' FAILED" >&2
  failed=1
fi
...
node_out=$(node tests/runner-vault-classifier-twitter.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'vault-classifier-twitter-collector' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-extension-settings-ops.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'extension-settings-ops' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-content-x-blackout.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
...
node_out=$(node tests/runner-site-path-blocking.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'site-path-blocking' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-popup-boot.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
...
node_out=$(node tests/runner-page-exit.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'page-exit' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-content-cover.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'content-cover' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-quick-add.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'quick-add' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-enforce-only.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'enforce-only' FAILED" >&2
  failed=1
fi
...
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'usage-once' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-parental-pin.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'parental-pin' FAILED" >&2
  failed=1
fi
...
node_out=$(node tests/runner-content-page-verdict.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
...
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
  echo "[run.sh] suite 'content-tag-page-filter' FAILED" >&2
  failed=1
fi
node_out=$(node tests/runner-tag-filter-syntax.js 2>&1) || failed=1
echo "$node_out"
if ! echo "$node_out" | grep -q "__CB_TEST_RESULT__: OK"; then
